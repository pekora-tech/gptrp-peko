...
AI 代理不僅根據當下環境做出反應，更能**記住過去的經驗**、**規劃未來的目標**，模擬真實生物的認知和決策過程。
...
![1767599030553](image/readme/1767599030553.png)
...
2. 📡 透過 WebSocket 發送到後端
   ↓
   { type: "requestNextMove", agent_id: "player", position: {...}, ... }

3. 🗄️ 後端從數據庫檢索記憶
   ↓
   ├─ 📝 短期記憶: 最近 10 個行動
   ├─ 🧠 長期記憶: 重要事件（importance ≥7）
   ├─ 🗺️ 位置記憶: 附近已探索的位置
   ├─ 🎮 互動記憶: 最近的互動行為
   └─ 🎯 當前目標: 活躍的目標（如有）
...
7. 📝 解析 JSON 回應
   ↓
   {
     "action": { "type": "navigate", "x": 7, "y": 6 },
     "reasoning": "我很累了，記得床在 (7,6)，去那裡睡覺",
     "new_goal": "到達床並休息"  // 可選
   }
...
1. **記憶檢索** - 每次決策前從數據庫加載相關記憶
2. **目標追蹤** - AI 知道當前目標並優先完成
3. **經驗積累** - 決策和結果都會保存，形成經驗
4. **持久化** - 重啟後記憶和目標保留
5. **智能推理** - AI 可基於過去經驗做出更明智的決策
...
本專案已成功實現完整的記憶與目標系統（階段 1），包括：

#### 核心組件

- ✅ **Database.js** - SQLite 數據庫封裝，支持四種記憶類型和目標管理
- ✅ **MemoryManager.js** - 統一的記憶管理接口，處理記憶的 CRUD 操作
- ✅ **GoalManager.js** - 目標生命週期管理，自動達成檢測
- ✅ **ServerAgent.js** - 整合記憶、目標和 OpenAI 的核心決策引擎

#### 測試套件

- ✅ **test-database.js** - 數據庫連接和基礎操作測試
- ✅ **test-memory.js** - 四種記憶類型的完整測試
- ✅ **test-goals.js** - 目標系統的生命週期測試
- ✅ **test-integration.js** - 端到端整合測試

#### 技術特點

- 🔹 **持久化存儲**: 所有記憶和目標保存在 SQLite，重啟後保留
- 🔹 **索引優化**: 數據庫查詢速度 <50ms
- 🔹 **記憶分層**: 短期/長期記憶分離，智能篩選重要信息
- 🔹 **目標驅動**: AI 可自主設定、追蹤和完成目標
- 🔹 **容錯設計**: 即使記憶系統失效，仍可降級到基礎模式運行

### 後續計劃

- **階段 2** - 標籤解析架構，支持 Ollama 本地模型
- **階段 3** - NPC 系統，實現多個 AI 動物代理
...
- **OpenAI** - 提供強大的 AI 能力
- **Tiled** - 直觀的地圖編輯工具
- **better-sqlite3** - 高效的同步 SQLite 封裝
